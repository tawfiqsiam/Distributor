...
client.on('message', msg =>{
    if (msg.content === '=help'){
         ...
         .addField("__Some more accounts__ \n =crunchyroll/hulu/netflix: Crunchyroll, Hulu & Netflix \n =hulu/origin: Hulu/Origin" , "More accounts coming soon! Join the support server for more info on restocks: https://discord.gg/e9kzWCE")
...
client.on('message', message =>{
    if (!message.guild) return;
if (message.content === '=hulu/origin'){
    if (usedCommandRecently4.has(message.author.id)){
        message.channel.send('`Cooldown.. Please let other people get accounts.`')
    } else{
        usedCommandRecently4.add(message.author.id);
    ...
    var string = 
    `https://up-to-down.net/43300/00014  If the account doesnt work it means someone already claimed it.`
    var words = string.split('\n');
    let random = words[Math.floor(Math.random()*words.length)];
    message.reply("Check your DMs")
    message.author.send(`${random}`);      
